...
exports.fetchChannelData = exports.getYoutubeInstance = void 0;
...
exports.getYoutubeInstance = getYoutubeInstance;
const fetchChannelData = async (url) => {
    const yt = await (0, exports.getYoutubeInstance)();
    ...
    try {
        ...
        if (lockups.length > 0) {
            videos = lockups.map((l) => {
                ...
                const bottomOverlay = l.content_image?.overlays?.find((o) => o.type === 'ThumbnailBottomOverlayView');
                ...
                let isMemberOnly = accessibilityLabel.toLowerCase().includes("members only") ||
                    ...
                    badges.some((b) => JSON.stringify(b).toLowerCase().includes("members"));
...
function parseRelativeToMs(relativeText) {
    const now = Date.now();
    if (!relativeText)
        return 0; // If missing, send to bottom of list
    ...
    if (!match)
        return 0; // If parsing fails, send to bottom
...
function parseAttachment(attachment) {
    ...
    if (attachment.type === "PostMultiImage") {
        return {
            type: "images",
            images: (attachment.images || []).map((img) => getBestUrl(img.image)).filter(Boolean)
        };
    }
